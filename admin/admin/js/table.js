jQuery(document).ready(function () {
    if ($.fn.editable) {
        // 快速编辑的url提交地址
        $.fn.editable.defaults.url = Quick.quick_edit_url;
        // 值为空时显示的信息
        ...
        $.fn.editable.defaults.params = function (params) {
            ...
            //params.validate = Quick.validate;
            //params.validate_fields = Quick.validate_fields;
            return params;
        };
        // 提交成功时的回调函数
    ...
    var goto = function (url, _curr_params, remove_page) {
        ...
        if (remove_page && Quick.curr_params['page'] !== undefined) {
            delete Quick.curr_params['page'];
        }

        if ($.isEmptyObject(Quick.curr_params)) {
            params = jQuery.param(_curr_params);
        } else {
            $.extend(Quick.curr_params, _curr_params);
            params = jQuery.param(Quick.curr_params);
        }
    ...
    // 初始化搜索
    var search_field = Quick.search_field;
    ...
    $('.table-builder .field-filter').click(function () {
        var self             = $(this),
            ...
            $_filter         = Quick._filter,
            $_filter_content = Quick._filter_content,
            $_field_display  = Quick._field_display,
            $data  = {
        ...
        layer.open({
            ...
            success: function () {
                ...
                // 获取数据
                $.post(Quick.get_filter_list, $data).success(function(res) {
                    if (1 !== res.code) {
            ...
            yes: function () {
                ...
                goto(Quick.curr_url, _curr_params, true);
            }
        });
        return false;
    });
    ...
    $('body').delegate('#check-all', 'click', function () {
        var $checkStatus = $(this).prop('checked');
        if ($('.js-table-checkable').val()) {
            $('.js-table-checkable input[name="ids[]"]').each(function () {
                $(this).prop('checked', $checkStatus);
            });
        } else {
            $('.js-table-checkable input[name="ids[]"]').each(function () {
    ...
    $('.table-builder .switch input:checkbox').on('click', function () {
        ...
        var $data = {
            ...
            type: 'switch',
            pk: $switch.data('id') ? $switch.data('id') : $switch.data('pk')
        };

        // 发送ajax请求
        pageLoader();
        $.post(Quick.quick_edit_url, $data).success(function(res) {
            pageLoader('hide');
            if (1 != res.code) {
                tips(res.msg, 'danger');
    ...
    $('#go-page').on('keyup', function (e) {
        if (e.keyCode === 13) {
            ...
            goto(Quick.curr_url, _curr_params);
        }
    });
    ...
    $('a.pop').click(function () {
        ...
        // 处理各种回调方法
        Quick.layer.success = Quick.layer.success ? window[Quick.layer.success] : null;
        Quick.layer.yes     = Quick.layer.yes ? window[Quick.layer.yes] : null;
        Quick.layer.cancel  = Quick.layer.cancel ? window[Quick.layer.cancel] : null;
        Quick.layer.end     = Quick.layer.end ? window[Quick.layer.end] : null;
        Quick.layer.full    = Quick.layer.full ? window[Quick.layer.full] : null;
        Quick.layer.min     = Quick.layer.min ? window[Quick.layer.min] : null;
        Quick.layer.max     = Quick.layer.max ? window[Quick.layer.max] : null;
        Quick.layer.restore = Quick.layer.restore ? window[Quick.layer.restore] : null;

        if ($layer !== undefined) {
            // 处理各种回调方法
            $layer.success = $layer.success ? window[$layer.success] : Quick.layer.success;
            $layer.yes     = $layer.yes ? window[$layer.yes] : Quick.layer.yes;
            $layer.cancel  = $layer.cancel ? window[$layer.cancel] : Quick.layer.cancel;
            $layer.end     = $layer.end ? window[$layer.end] : Quick.layer.end;
            $layer.full    = $layer.full ? window[$layer.full] : Quick.layer.full;
            $layer.min     = $layer.min ? window[$layer.min] : Quick.layer.min;
            $layer.max     = $layer.max ? window[$layer.max] : Quick.layer.max;
            $layer.restore = $layer.restore ? window[$layer.restore] : Quick.layer.restore;

            $.extend($options, Quick.layer, $layer);
        } else {
            $.extend($options, Quick.layer);
        }
