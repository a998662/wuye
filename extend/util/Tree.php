...
class Tree
{
    ...
    protected static $config = [
        'id'    => 'id',    // id名称
        'parent_id'   => 'parent_id',   // parent_id名称
        'title' => 'title', // 标题名称
        'child' => 'child', // 子元素键名
        'html'  => '┝ ',   // 层级标记
        'step'  => 4,       // 层级步进数量
    ...
    public static function toLayer($lists = [], $parent_id = 0, $max_level = 0, $curr_level = 0)
    {
        ...
        foreach ($lists as $key => $value) {
            if ($value[self::$config['parent_id']] == $parent_id) {
     ...
     */
    public static function toList($lists = [], $parent_id = 0, $level = 0)
    {
        if (is_array($lists)) {
            $trees = [];
            foreach ($lists as $key => $value) {
            ...
            foreach ($lists as $key => $value) {
                if ($value[self::$config['parent_id']] == $parent_id && is_object($value)) {
                    $title_prefix   = str_repeat("&nbsp;", $level * self::$config['step']).self::$config['html'];
                    $value['level'] = $level + 1;
                    $value['title_prefix']  = $level == 0 ? '' : $title_prefix;
                    $value['title_display'] = $level == 0 ? $value[self::$config['title']] : $title_prefix.$value[self::$config['title']];
    ...
    public static function getParents($lists = [], $id = '')
    {
        ...
        foreach ($lists as $value) {
            if ($value[self::$config['id']] == $id) {
                $trees[] = $value;
                $trees   = array_merge(self::getParents($lists, $value[self::$config['parent_id']]), $trees);
            }
        }
        return $trees;
    }
     ...
     * @return array
     */
    public static function getChildsId($lists = [], $parent_id = '')
    {
        $result = [];
        foreach ($lists as $value) {
            if ($value[self::$config['parent_id']] == $parent_id) {
     ...
     * @return array
     */
    public static function getChilds($lists = [], $parent_id = '')
    {
        $result = [];
        foreach ($lists as $value) {
            if ($value[self::$config['parent_id']] == $parent_id) {
