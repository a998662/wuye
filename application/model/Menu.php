...
class Menu extends Model
{
    ...
    {
        if ($id > 0) {
            $ids = self::where('parent_id', $id)->column('id');
            if ($ids) {
                foreach ($ids as $id) {
    ...
    public static function getMenuTree($id = 0, $default = '', $module = '')
    {
        ...
        // 获取节点
        $menus = Tree::toList(self::where($where)->order('parent_id,id')->column('id,parent_id,title'));
    ...
    public static function getTopMenu($max = '', $cache_tag = '')
    {
        ...
        if (!$menus) {
            ...
            $map['status'] = 1;
            $map['parent_id']    = 0;
            $menus = self::where($map)->order('sort,id')->limit($max)->column('id,parent_id,module,title,url_value,url_type,url_target,icon,params');
    ...
    public static function getSidebarMenu($id = '', $module = '', $controller = '')
    {
        ...
        if (!$menus) {
            ...
            $menus = self::where($map)->order('sort,id')->column('id,parent_id,module,title,url_value,url_type,url_target,icon,params');
    ...
    public static function getLocation($id = '', $del_last_url = false, $check = true)
    {
        ...
        if (!$location) {
            $map[] = ['parent_id','<>', 0];
            $map[]= ['url','=',strtolower('/'.$controller.'/'.$action)];

            // 当前操作对应的节点ID
            $curr_id  = $id == '' ? self::where($map)->order('parent_id desc')->value('id') : $id;

            // 获取节点ID是所有父级节点
            $location = Tree::getParents(self::order('parent_id')->column('id,parent_id,name,url'), $curr_id);
     ...
     * @return array
     */
    public static function getChildsId($parent_id = 0)
    {
        $ids = self::where('parent_id', $parent_id)->column('id');
    ...
    public static function getParentsId($id = 0)
    {
        ...
        if ($parent_id != 0) {
            ...
            $parent_ids = array_merge($parent_ids, self::getParentsId($parent_id));
        }
        return $parent_ids;
    }
